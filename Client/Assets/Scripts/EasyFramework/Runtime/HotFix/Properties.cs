...
public class Properties : MonoBehaviour
{
    ...
    {
        foreach (var resultPair in Pairs)
        {
            if (resultPair.key == key)
            {
                return (T)resultPair.value;
            }
        }
        return default(T);
    }
    ...
    [SerializeField] private String[] StringPairs;
    [SerializeField] private Number[] NumberPairs;
    [SerializeField] private Boolean[] BooleanPairs;
    [SerializeField] private Object[] ObjectPairs;
    [SerializeField] private ObjectArray[] ObjectArrayPairs;
    [SerializeField] private GameObjectArray[] GameObjectArrayPairs;
...
namespace UnityEditor.GUI
{
    ...
    public class Properties_CustomEditor : Editor
    {
        private Object _scriptObject;

        //实例引用
        private Properties _instance;
        ...
        void OnEnable()
        {
            //Debug.Log("OnEnable");
            _scriptObject = null;
            _instance = target as Properties;
            if (_instance == null)
                return;

            //script object
            var targetName = $"/{nameof(Properties)}.cs";
            var targetPath = AssetDatabase.GetAllAssetPaths().FirstOrDefault(p =>
                p.EndsWith(targetName) && AssetDatabase.GetMainAssetTypeAtPath(p) == typeof(MonoScript));
            ...
            display.AddDrawer<StringDrawer>(typeof(Properties.String));
            display.AddDrawer<NumberDrawer>(typeof(Properties.Number));
            display.AddDrawer<BooleanDrawer>(typeof(Properties.Boolean));
            display.AddDrawer<ObjectDrawer>(typeof(Properties.Object));
            display.AddDrawer<ObjectArrayDrawer>(typeof(Properties.ObjectArray));
            display.AddDrawer<GameObjectArrayDrawer>(typeof(Properties.GameObjectArray));
        ...
        private DrawerInfo CreateDrawerInfo()
        {
            ...
            if (_instance != null)
            {
                ...
                var fields =
                    typeof(Properties).GetFields(BindingFlags.NonPublic | BindingFlags.Instance |
                                                   BindingFlags.Public);
                foreach (var field in fields)
                {
                    Type elementType = field.FieldType.IsArray ? field.FieldType.GetElementType() : null;
                    if (elementType != null
                        && typeof(Properties.IPair).IsAssignableFrom(elementType)
                        && serializedObject.FindProperty(field.Name) != null
                        && DisplayUtility.CheckFields(elementType))
                    {

                        var options = elementType.GetCustomAttribute<Properties.OptionsAttribute>(false);
        ...
        void DisplayMenu()
        {
            ...
            if (menuFoldout)
            {
                ...
                {
                    Properties.ResultPair[] pairs = _instance.GenPairs();
                    if (pairs != null)
                    {
                        ...
                        for (int i = 0; i < pairs.Length; i++)
                        {
                            ...
                            if (checkKeyRedefinition)
                            {
                                ...
                                {
                                    EditorGUILayout.BeginHorizontal();
                                    EditorGUILayout.LabelField("", "", "CN EntryWarnIconSmall",
                                        GUILayout.Width(20));
                                    EditorGUILayout.LabelField(
                                        $"redefinition key at {i} (other at {keys.IndexOf(key)})");
                                    EditorGUILayout.EndHorizontal();
                                }
                                else keys.Add(key);
                            }
    ...
    public class Element
    {
        ...
        static void CleanNode(SerializedProperty node)
        {
            ...
            var prop = (from _p in typeof(SerializedProperty).GetProperties(BindingFlags.Instance |
                    BindingFlags.Public)
            ...
            if (prop != null)
                ...
                throw new KeyNotFoundException(string.Format("expected prop name = {0}, but not found.",
                    expectedPropName));
        }
    ...
    public class DisplayUtility
    {
        ...
        public static void PopupComponentsAndTypes(DrawerInfo info, Element element, Object obj, Type targetType)
        {
            ...
            if (objects != null)
            {
                ...
                _options.AddRange(
                    DisplayUtility.CheckOptions((from o in objects select ("-" + o.GetType().Name)).ToArray()));
            ...
            DisplayUtility.CustomMenu(options, separator, selected, selected, (value) =>
            {
                if (objects == null || value >= objects.Length)
                {
                    ...
                    {
                        create.valueNode.arraySize = element.valueNode.isArray ? element.valueNode.arraySize : 0;
                        create.valueNode.isExpanded =
                            element.valueNode.isArray ? element.valueNode.isExpanded : true;
                    }
        ...
        {
            FieldInfo index, key;
            return typeof(Properties.IPair).IsAssignableFrom(type)
        ...
        public static void Copy(Element from, Element to)
        {
            if (from.valueType.IsArray && to.valueType.IsArray)
            {
                ...
                {
                    Copy(from.valueNode.GetArrayElementAtIndex(i), to.valueNode.GetArrayElementAtIndex(i),
                        targetType);
                }
            }
            else
            ...
            if (fromNode.propertyType == toNode.propertyType)
            {
                switch (fromNode.propertyType)
                {
                    ...
                    case SerializedPropertyType.ObjectReference:
                        if (fromNode.objectReferenceValue != null &&
                            targetType.IsAssignableFrom(fromNode.objectReferenceValue.GetType()))
        ...
        public static void CustomMenu(string[] options, string[] separator, int[] selected, int[] disabled,
            Action<int> callback)
        {
        ...
        static Object[] GetCompoents(Object obj, Type targetType)
        {
            if (obj != null)
            {
                ...
                {
                    var components = getComponents.Invoke(gameObject != null ? gameObject : obj,
                        new object[] {typeof(Component)}) as Component[];
        ...
        public static string GenCode(Properties.ResultPair[] pairs, string declareType, bool useFullname)
        {
            var resultCode = "";
        ...
        public static void PraseCode(Properties_CustomEditor editor, string code)
        {
            ...
            foreach (var line in code.Split(';'))
            {
                ...
                if (columns.Length <= 0)
                    continue;
                var declareKey = columns[0].Split(' ').Select(o => o.Trim()).Where(o => !string.IsNullOrEmpty(o))
                    .ToArray();
                ...
                //Create Element
                var arrayParent =
                    editor.drawInfo.GetSerializedProperty(editor.drawInfo.typeMapping.Keys.ElementAt(valueType));
                arrayParent.arraySize++;
                var create = new Element(arrayParent.GetArrayElementAtIndex(arrayParent.arraySize - 1),
                    editor.drawInfo.typeMapping.Values.ElementAt(valueType));
        ...
        static string GetTypeName(Type type, bool useFullname)
        {
            ...
            //Value Mapping
            if (type.Equals(typeof(double)) || type.Equals(typeof(float)) || type.Equals(typeof(int)) ||
    ...
    public abstract class ArrayDrawer : Drawer
    {
        ...
        protected override void DrawValue(Rect position, Rect valueRect)
        {
            ...
            var newState = EditorGUI.Foldout(new Rect(valueRect) {x = valueRect.x + 10}, arrayParent.isExpanded,
                title);
        ...
        private bool DrawElement(Rect position, int index, SerializedProperty node)
        {
            ...
            {
                DisplayUtility.PopupComponents(drawInfo, node, node.objectReferenceValue,
    ...
    class NumberDrawer : Drawer
    {
        protected override void DrawValue(Rect position, Rect valueRect)
        {
            element.valueNode.doubleValue =
                EditorGUI.DoubleField(valueRect, string.Empty, element.valueNode.doubleValue);
        }
    }
    ...
    class StringDrawer : Drawer
    {
        protected override void DrawValue(Rect position, Rect valueRect)
        {
            element.valueNode.stringValue =
                EditorGUI.TextField(valueRect, string.Empty, element.valueNode.stringValue);
        }
    }
    ...
    class ObjectDrawer : Drawer
    {
        protected override void DrawValue(Rect position, Rect valueRect)
        {
            element.valueNode.objectReferenceValue = EditorGUI.ObjectField(valueRect, string.Empty,
                element.valueNode.objectReferenceValue, typeof(Object), true);
        }
        ...
        protected override void DrawOptions(Rect optionsRect)
        {
            if (EditorGUI.Toggle(optionsRect, false, OPTIONS_STYLE))
            {
                DisplayUtility.PopupComponentsAndTypes(drawInfo, element, element.valueNode.objectReferenceValue,
                    element.valueType);
            }
        }
    }
    ...
    class ObjectArrayDrawer : ArrayDrawer
    {
        ...
        {
            node.objectReferenceValue = EditorGUI.ObjectField(valueRect, string.Empty, node.objectReferenceValue,
                typeof(Object), true);
        }
    }
